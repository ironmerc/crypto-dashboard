...
export default function TelegramSettings() {
    ...
    return (
        <div className="h-screen bg-terminal-bg text-terminal-fg/80 font-mono flex flex-col overflow-hidden">
            {/* Header */}
            <div className="flex-none p-4 border-b border-terminal-border bg-terminal-surface/50 flex flex-col sm:flex-row sm:justify-between items-start sm:items-center gap-4 sm:gap-0 z-10 w-full">
                <div className="flex items-center space-x-3 w-full sm:w-auto">
                    <ShieldAlert className="w-5 h-5 text-terminal-blue shrink-0" />
                    <div>
                        <h1 className="text-lg sm:text-xl font-bold text-terminal-fg tracking-tight truncate">Telegram Operations</h1>
                        <p className="text-[10px] text-terminal-muted/70 font-mono hidden sm:block">Mission Control & Strategy Egress</p>
                    </div>
                </div>
                <div className="flex items-center w-full sm:w-auto justify-end space-x-4">
                    <div className="flex bg-terminal-bg/50 p-1 rounded-lg border border-terminal-border">
                        <button
                            onClick={() => setActiveTab('general')}
                            className={`px-3 py-1.5 rounded-md text-xs font-bold transition-all ${activeTab === 'general' ? 'bg-terminal-blue text-terminal-fg shadow-lg' : 'text-terminal-muted hover:text-slate-200'}`}
                        >
                            General
                        </button>
                        <button
                            onClick={() => setActiveTab('alerts')}
                            className={`px-3 py-1.5 rounded-md text-xs font-bold transition-all ${activeTab === 'alerts' ? 'bg-terminal-blue text-terminal-fg shadow-lg' : 'text-terminal-muted hover:text-slate-200'}`}
                        >
                            Alerts
                        </button>
                        <button
                            onClick={() => setActiveTab('diagnostics')}
                            className={`px-3 py-1.5 rounded-md text-xs font-bold transition-all ${activeTab === 'diagnostics' ? 'bg-terminal-blue text-terminal-fg shadow-lg' : 'text-terminal-muted hover:text-slate-200'}`}
                        >
                            Logs
                        </button>
                    </div>
                    <Link to="/" className="flex items-center space-x-2 text-terminal-muted hover:text-terminal-fg transition-colors text-sm bg-terminal-border/50 px-3 py-1.5 rounded-md border border-terminal-border/80 hover:border-slate-500 whitespace-nowrap">
                                ...
                                <div className="space-y-8">
                                    <section>
                                        <h3 className="text-[10px] font-bold text-terminal-muted/70 uppercase tracking-[0.2em] mb-4 flex items-center">
                                            <Activity className="w-3.5 h-3.5 mr-2 text-terminal-blue" /> Connection Status
                                        </h3>
                                        <div className="bg-terminal-surface/40 border border-terminal-border/60 rounded-xl p-6 backdrop-blur-sm">
                                            <div className="space-y-6">
                                                <div className="flex justify-between items-center">
                                                    <span className="text-terminal-muted text-sm">Backend Link</span>
                                                ...
                                                <div className="pt-4 border-t border-terminal-border/50 flex justify-between items-center">
                                                    <span className="text-terminal-muted text-sm">Target Identity</span>
                                                    <span className="text-terminal-blue font-mono font-bold">@{status.bot_username || '---'}</span>
                                                </div>
                                            </div>
                                        </div>
                                    </section>
                                    ...
                                    <section>
                                        <h3 className="text-[10px] font-bold text-terminal-muted/70 uppercase tracking-[0.2em] mb-4 flex items-center">
                                            <Settings className="w-3.5 h-3.5 mr-2 text-terminal-blue" /> Strategy Egress
                                        </h3>
                                        <div className="bg-terminal-surface/40 border border-terminal-border/60 rounded-xl p-6 backdrop-blur-sm space-y-6">
                                            <div className="flex items-center justify-between">
                                                <div>
                                                    <h4 className="text-terminal-fg font-bold text-sm">Master Broadcast Toggle</h4>
                                                    <p className="text-[10px] text-terminal-muted/70 mt-1 max-w-[200px]">Globally enable or disable all outgoing Telegram alerts.</p>
                                                </div>
                                            ...
                                            <div className="pt-6 border-t border-terminal-border/50 flex items-center justify-between">
                                                <div>
                                                    <h4 className="text-terminal-fg font-bold text-sm">System State Shift Alerts</h4>
                                                    <p className="text-[10px] text-terminal-muted/70 mt-1 max-w-[200px]">Alert when global market regime or volatility regime changes.</p>
                                                </div>
                                                <label className="relative inline-flex items-center cursor-pointer scale-110">
                                                    <input
                                                        type="checkbox"
                                        ...
                                        <div className="bg-terminal-surface/40 border border-terminal-border/60 rounded-xl p-6 backdrop-blur-sm">
                                            <div className="flex space-x-2 mb-4">
                                                <input
                                                    ...
                                                    className="flex-1 bg-terminal-bg border border-terminal-border rounded-lg px-3 py-2 text-xs text-slate-200 focus:border-terminal-blue transition-colors placeholder:text-terminal-muted/50 outline-none"
                                                />
                                                <button
                                                    onClick={addSymbol}
                                                    className="px-4 py-2 bg-terminal-blue hover:bg-terminal-blue text-terminal-fg rounded-lg text-xs font-bold transition-all shadow-lg active:scale-95"
                                                >
                                                    Add
                                                </button>
                                            </div>
                                            <div className="flex flex-wrap gap-2">
                                                {(!config.monitoredSymbols || config.monitoredSymbols.length === 0) ? (
                                                    ...
                                                    config.monitoredSymbols.map(s => (
                                                        <div key={s} className="flex items-center bg-terminal-bg p-2 rounded-lg border border-terminal-border group hover:border-terminal-blue/50 transition-colors">
                                                            <span className="text-[11px] font-bold text-terminal-fg/80 px-2">{s}</span>
                                                            <button onClick={() => removeSymbol(s)} className="p-1 text-terminal-muted/50 hover:text-red-400 transition-colors rounded-md bg-terminal-surface group-hover:bg-terminal-border/30">
                                        ...
                                        <div className="bg-terminal-surface/40 border border-terminal-border/60 rounded-xl p-6 backdrop-blur-sm">
                                            <div className="flex items-center justify-between mb-6">
                                                <div>
                                                    ...
                                                    <p className="text-[10px] text-terminal-muted/70 mt-1">Suppress alerts during specific windows.</p>
                                                </div>
                                                <label className="relative inline-flex items-center cursor-pointer">
                                                    ...
                                                    />
                                                    <div className="w-9 h-5 bg-slate-700 rounded-full peer peer-checked:after:translate-x-full after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:bg-terminal-blue"></div>
                                                </label>
                                            </div>
                                                ...
                                                <div className="grid grid-cols-2 gap-4">
                                                    <div>
                                                        ...
                                                        <input
                                                            ...
                                                            className="w-full bg-terminal-bg border border-terminal-border rounded-lg px-3 py-2 text-xs text-terminal-blue outline-none focus:border-terminal-blue"
                                                        />
                                                    </div>
                                                    <div>
                        ...
                        {activeTab === 'alerts' && (
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                                ...
                                <section className="space-y-4">
                                    ...
                                    <div className="bg-terminal-surface/40 border border-terminal-border/60 rounded-xl p-6 backdrop-blur-sm h-full">
                                        <div className="space-y-6">
                                            ...
                                            {['oi_spike', 'atr_expand', 'liquidation', 'whale', 'extreme_funding', 'ema_cross', 'rsi_extreme', 'rvol_spike', 'order_flow', 'level_testing', 'context_summary', 'market_context'].map(id => (
                                                <div key={id} className="p-4 bg-terminal-bg/50 border border-terminal-border/50 rounded-xl space-y-4">
                                                    <div className="flex items-center justify-between">
                                                        <div className="flex items-center space-x-3">
                                                            <div className={`w-2 h-2 rounded-full ${(config.categories?.[id] ?? true) ? 'bg-terminal-blue shadow-[0_0_8px_rgba(99,102,241,0.5)]' : 'bg-slate-700'}`}></div>
                                                    ...
                                                    <div className="flex flex-wrap items-center gap-4 pt-4 border-t border-terminal-border/30">
                                                                    ...
                                                                    return (
                                                                        <button
                                                                            ...
                                                                            className={`px-2 py-1 rounded text-[10px] font-mono border transition-all ${isActive
                                                                                ? "bg-terminal-blue/20 border-terminal-blue/50 text-indigo-300 ring-1 ring-terminal-blue/20 shadow-sm"
                                                                                : "bg-terminal-surface border-terminal-border text-terminal-muted/70 hover:border-terminal-border/80 hover:text-terminal-muted"
                                                                                }`}
                                                                        >
                                                                            {tf}
                                                                        </button>
                                                        ...
                                                        <div className="w-20">
                                                            ...
                                                            <select
                                                                ...
                                                                className="w-full bg-terminal-surface border border-terminal-border rounded px-1.5 py-1 text-[10px] text-terminal-blue font-bold outline-none hover:border-terminal-blue/50 transition-colors"
                                                            >
                                ...
                                <section className="space-y-4">
                                    ...
                                    <div className="bg-terminal-surface/40 border border-terminal-border/60 rounded-xl p-6 backdrop-blur-sm">
                                        <div className="space-y-6">
                                            <div className="mb-6">
                                                <label className="block text-[10px] font-bold text-terminal-muted/70 mb-2 uppercase tracking-wider">Edit Target</label>
                                                <select
                                                    value={editingSymbol}
                                                    onChange={(e) => setEditingSymbol(e.target.value)}
                                                    className="w-full bg-terminal-bg border border-terminal-border rounded px-2 py-2 text-xs text-terminal-blue focus:border-terminal-blue outline-none"
                                                >
                        ...
                        {activeTab === 'diagnostics' && (
                            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 animate-in fade-in slide-in-from-bottom-4 duration-500 h-[calc(100vh-250px)]">
                                ...
                                <section className="lg:col-span-1 space-y-4">
                                    ...
                                    <div className="bg-terminal-surface/40 border border-terminal-border/60 rounded-xl p-6 backdrop-blur-sm">
                                        <p className="text-[11px] text-terminal-muted mb-4 h-12">Execute diagnostic egress to verify bot connectivity and payload rendering.</p>
                                        <div className="space-y-4">
                                            <select
                                                value={selectedTest}
                                                onChange={(e) => setSelectedTest(e.target.value)}
                                                className="w-full bg-terminal-bg border border-terminal-border rounded px-2 py-2 text-xs text-terminal-fg/80 focus:border-terminal-blue focus:outline-none"
                                            >
                                            ...
                                            <button
                                                ...
                                                className="w-full bg-terminal-blue hover:bg-terminal-blue disabled:opacity-50 text-terminal-fg text-xs font-bold py-2.5 rounded-lg border border-terminal-blue/50 shadow-lg active:scale-95 transition-all flex items-center justify-center space-x-2"
                                            >
                                ...
                                <section className="lg:col-span-2 space-y-4 flex flex-col min-h-0">
                                    <div className="flex justify-between items-center mb-0">
                                        ...
                                        <span className="text-[10px] text-terminal-muted/70 bg-terminal-surface/50 px-2 py-0.5 rounded border border-terminal-border">Latest 50</span>
                                    </div>

                                    <div className="bg-terminal-surface/40 border border-terminal-border/60 rounded-xl p-4 backdrop-blur-sm flex-1 overflow-y-auto custom-scrollbar">
                                        {(!history || history.length === 0) ? (
                                            ...
                                            <div className="space-y-3">
                                                {history.map((item, i) => (
                                                    <div key={i} className="bg-terminal-bg/80 border border-terminal-border rounded-lg p-3 text-[11px] animate-in fade-in slide-in-from-right-2 duration-300">
                                                        <div className="flex justify-between items-start mb-2">
                                                            <div className="flex items-center space-x-2">
                                                                <span className={`px-1.5 py-0.5 rounded text-[9px] font-bold uppercase border ${item.severity === 'critical' ? 'bg-red-500/10 text-red-400 border-red-500/20' : 'bg-terminal-blue/10 text-terminal-blue border-terminal-blue/20'
                                                                    }`}>
                                                                    {item.category}
                                                                </span>
                                                                <span className="font-bold text-slate-200">{item.symbol}</span>
                                                            </div>
                                                            <span className="text-terminal-muted/50 font-mono text-[9px]">
                                                                {new Date(item.timestamp).toLocaleTimeString()}
                                                            </span>
                                                        </div>
                                                        <p className="text-terminal-muted leading-relaxed font-mono line-clamp-2">{item.message}</p>
                                                    </div>
                                                ))}
                                            </div>
                                        )}
